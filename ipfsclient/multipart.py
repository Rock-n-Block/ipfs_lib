...
def multipart_content_type_headers(boundary, subtype="mixed"):
    ...
    """
    ctype = 'multipart/%s; boundary="%s"' % (subtype, boundary)
...
def content_type_headers(filename, content_type=None):
    ...
    """
    return {
        "Content-Type": content_type if content_type else utils.guess_mimetype(filename)
    }
...
def content_disposition_headers(filename, disptype="form-data"):
    ...
    """
    disp = '%s; filename="%s"' % (disptype, urllib.parse.quote(filename, safe=""))
...
class StreamFileMixin(object):
    def _gen_file(self, filename, file_location=None, file=None, content_type=None):
        ...
        for chunk in self._gen_file_start(filename, file_location, content_type):
            yield chunk
        if file:
            for chunk in self._gen_file_chunks(file):
                yield chunk
        for chunk in self._gen_file_end():
            yield chunk
    ...
    def _gen_file_start(self, filename, file_location=None, content_type=None):
        ...
        """
        # PY2: Use `yield from` instead
        for chunk in self._gen_item_start():
            yield chunk

        headers = content_disposition_headers(
            filename.replace(os.sep, "/"), disptype="file"
        )
        ...
        # PY2: Use `yield from` instead
        for chunk in self._gen_headers(headers):
            yield chunk
...
class StreamBase(object):
    ...
    def __init__(self, name, chunk_size=default_chunk_size):
        ...
        self._headers.update(
            multipart_content_type_headers(self._boundary, subtype="form-data")
        )
    ...
    def body(self, *args, **kwargs):
        ...
        # PY2: Use `yield from` instead
        for chunk in self._gen_chunks(self._body()):
            yield chunk
...
class DirectoryStream(StreamBase, StreamFileMixin):
    ...
    def __init__(
        ...
        for pattern in patterns:
            self.patterns.append(
                glob_compile(pattern) if isinstance(pattern, str) else pattern
            )
    ...
    def _body_directory(self, short_path, visited_directories):
        ...
        while dir_parts:
            ...
            # PY2: Use `yield from` instead
            for chunk in self._gen_file(
                dir_base, content_type="application/x-directory"
            ):
                yield chunk
    ...
    def _body_file(self, short_path, file_location, dir_fd=-1):
        try:
            if dir_fd >= 0:
                f_path_or_desc = os.open(
                    file_location, os.O_RDONLY | os.O_CLOEXEC, dir_fd=dir_fd
                )
            else:
    ...
    def _body(self):
        ...
        if not isinstance(self.directory, int):
            ...
            if isinstance(directory, six.text_type) and not isinstance(
                sep, six.text_type
            ):  # PY2
                import sys

                sep = sep.decode(sys.getfilesystemencoding())
            elif isinstance(directory, six.binary_type) and not isinstance(
                sep, six.binary_type
            ):  # PY3 noqa
                sep = os.fsencode(sep)
            ...
            for filename in filenames:
                ...
                if wildcard_directory:
                    ...
                    for chunk in self._body_file(
                        short_file_path, file_location, dir_fd=dir_fd
                    ):
                        yield chunk
                else:
                    ...
                    if match_short_path(short_file_path):
                        # PY2: Use `yield from` instead
                        for chunk in self._body_directory(
                            short_path, visited_directories
                        ):
                            yield chunk
                        for chunk in self._body_file(
                            short_file_path, file_location, dir_fd=dir_fd
...
class BytesFileStream(FilesStream):
    ...
    def body(self):
        ...
        # PY2: Use `yield from` instead
        for chunk in self._gen_file_start(self.name):
            yield chunk
        for chunk in self._gen_chunks(self.data):
            yield chunk
        for chunk in self._gen_file_end():
            yield chunk
        for chunk in self._gen_end():
...
def stream_filesystem_node(
    filepaths, recursive=False, patterns="**", chunk_size=default_chunk_size
):
...
def stream_directory(
    directory, recursive=False, patterns="**", chunk_size=default_chunk_size
):
